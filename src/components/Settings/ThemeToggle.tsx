...
const SunIcon = ({ isActive }: ToggleIconProps) => {
    return (
        <svg
            ...
            fill="currentColor"
            className={`bi bi-brightness-low-fill size-6 lg:size-8 ${isActive ? 'fill-amber-400' : 'fill-stone-500'}`}
            viewBox="0 0 16 16"
        >
...
const MoonIcon = ({ isActive }: ToggleIconProps) => {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            className={`bi bi-moon-stars-fill size-4 lg:size-5 ${isActive ? 'fill-amber-200' : 'fill-stone-500'}`}
            viewBox="0 0 16 16"
        >
